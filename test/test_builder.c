...
void test_builder_report_set_button(void)
{
    ...
    report_set(&l, &r, HID_GAMEPAD_BUTTON,0, 1);   /* pressed */
    report_set(&l, &r, HID_GAMEPAD_BUTTON,7, 100); /* pressed */
    TEST_ASSERT_EQUAL_HEX8(0x81, r.data[0]); /* bit 0 + bit 7 */

    report_set(&l, &r, HID_GAMEPAD_BUTTON,0, 0);   /* released */
...
void test_builder_report_set_axis(void)
{
    ...
    report_set(&l, &r, HID_GAMEPAD_AXIS,0, 0x1234);
    TEST_ASSERT_EQUAL_HEX8(0x34, r.data[4]);
    TEST_ASSERT_EQUAL_HEX8(0x12, r.data[5]);

    report_set(&l, &r, HID_GAMEPAD_AXIS,2, -1);
...
void test_builder_report_matches_packed_struct(void)
{
    ...
    report_set(&l, &r, HID_GAMEPAD_BUTTON,0, 1);   /* button 0 */
    report_set(&l, &r, HID_GAMEPAD_BUTTON,2, 1);   /* button 2 */
    report_set(&l, &r, HID_GAMEPAD_AXIS,0, 1000);  /* x */
    report_set(&l, &r, HID_GAMEPAD_AXIS,1, -2000); /* y */
    report_set(&l, &r, HID_GAMEPAD_AXIS,2, 3000);  /* z */
    report_set(&l, &r, HID_GAMEPAD_AXIS,3, -4000); /* rz */
    report_set(&l, &r, HID_GAMEPAD_AXIS,4, 5000);  /* rx */
    report_set(&l, &r, HID_GAMEPAD_AXIS,5, -6000); /* ry */
...
void test_builder_scale_axis_endpoints(void)
{
    ...
    /* raw min → -32767 */
    report_set(&l, &r, HID_GAMEPAD_AXIS,0, 0);
    int16_t val = (int16_t)(r.data[0] | (r.data[1] << 8));
    TEST_ASSERT_EQUAL_INT16(-32767, val);

    /* raw max → 32767 */
    report_set(&l, &r, HID_GAMEPAD_AXIS,0, 4095);
...
void test_builder_scale_axis_midpoint(void)
{
    ...
    report_set(&l, &r, HID_GAMEPAD_AXIS,0, 500);
    int16_t val = (int16_t)(r.data[0] | (r.data[1] << 8));
...
void test_builder_scale_axis_signed_range(void)
{
    ...
    report_set(&l, &r, HID_GAMEPAD_AXIS,0, -100);
    int16_t val = (int16_t)(r.data[0] | (r.data[1] << 8));
    TEST_ASSERT_EQUAL_INT16(-32767, val);
...
void test_builder_hat_two_hats_packed(void)
{
    ...
    report_set(&l, &r, HID_GAMEPAD_HAT,0, 3); /* SE in low nibble → HID 3 */
    report_set(&l, &r, HID_GAMEPAD_HAT,1, 7); /* NW in high nibble → HID 7 */
...
void test_builder_hat_custom_raw_values(void)
{
    ...
    report_set(&l, &r, HID_GAMEPAD_HAT,0, 100);
    TEST_ASSERT_EQUAL_HEX8(0x00, r.data[0] & 0x0F);

    /* raw 300 → position 2 (S) */
    report_set(&l, &r, HID_GAMEPAD_HAT,0, 300);
    TEST_ASSERT_EQUAL_HEX8(0x02, r.data[0] & 0x0F);

    /* raw -1 → centered */
    report_set(&l, &r, HID_GAMEPAD_HAT,0, -1);
    TEST_ASSERT_EQUAL_HEX8(0x04, r.data[0] & 0x0F);

    /* Unknown raw value → centered (null) */
    report_set(&l, &r, HID_GAMEPAD_HAT,0, 999);
    TEST_ASSERT_EQUAL_HEX8(0x04, r.data[0] & 0x0F);
}
...
void test_builder_button_hysteresis(void)
{
    ...
    report_set(&l, &r, HID_GAMEPAD_BUTTON,0, 75);
    TEST_ASSERT_EQUAL_HEX8(0x00, r.data[0] & 0x01);
    /* Press it */
    report_set(&l, &r, HID_GAMEPAD_BUTTON,0, 100);
...
void test_builder_scale_button_threshold(void)
{
    ...
    report_set(&l, &r, HID_GAMEPAD_BUTTON,0, 1000);  /* below off */
    TEST_ASSERT_EQUAL_HEX8(0x00, r.data[0] & 0x01);

    report_set(&l, &r, HID_GAMEPAD_BUTTON,0, 3000);  /* above on */
    TEST_ASSERT_EQUAL_HEX8(0x01, r.data[0] & 0x01);

    report_set(&l, &r, HID_GAMEPAD_BUTTON,0, 0);     /* at min */
    TEST_ASSERT_EQUAL_HEX8(0x00, r.data[0] & 0x01);

    report_set(&l, &r, HID_GAMEPAD_BUTTON,0, 4095);  /* at max */
    TEST_ASSERT_EQUAL_HEX8(0x01, r.data[0] & 0x01);
}
...
void test_builder_switch_no_match_clears_all(void)
{
    ...
    report_set(&l, &r, HID_GAMEPAD_SWITCH,0, 10);
    TEST_ASSERT_EQUAL_HEX8(0x01, r.data[0]);
    report_set(&l, &r, HID_GAMEPAD_SWITCH,0, 999);
    TEST_ASSERT_EQUAL_HEX8(0x00, r.data[0]);
}
...
void test_builder_switch_with_buttons_offsets(void)
{
    ...
    report_set(&l, &r, HID_GAMEPAD_BUTTON,7, 1);
...
void test_builder_switch_out_of_range_is_noop(void)
{
    ...
    TEST_ASSERT_EQUAL_HEX8(0x01, r.data[0]);
    /* index 1 doesn't exist — should be a no-op */
    report_set(&l, &r, HID_GAMEPAD_SWITCH,1, 0);
    TEST_ASSERT_EQUAL_HEX8(0x01, r.data[0]);
}
...
void test_builder_two_switches(void)
{
    ...
    report_set(&l, &r, HID_GAMEPAD_SWITCH,0, 1);
    TEST_ASSERT_EQUAL_HEX8(0x01, r.data[0]);

    /* Switch B position 2 → bit 3 (btn_start=2, i=1 → idx=3) */
    report_set(&l, &r, HID_GAMEPAD_SWITCH,1, 200);
...
void test_builder_switch_between_buttons(void)
{
    ...
    report_set(&l, &r, HID_GAMEPAD_BUTTON,0, 1);
    TEST_ASSERT_EQUAL_HEX8(0x01, r.data[0]);
    ...
    report_set(&l, &r, HID_GAMEPAD_BUTTON,1, 1);
    TEST_ASSERT_EQUAL_HEX8(0x0B, r.data[0]); /* bits 0+1+3 */

    /* Release button 0 */
    report_set(&l, &r, HID_GAMEPAD_BUTTON,0, 0);
